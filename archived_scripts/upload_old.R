...
upload_ui <- function(id) {
  ...
  tagList(
    ...
    tabsetPanel(
      ...
      tabPanel(
        "Images",
        fileInput(ns("image_files"), "Add a new file",
                  multiple = TRUE,
                  accept = c("image/png", "image/gif", "image/jpg", "image/jpeg"),
                  buttonLabel = "Choose Files",
                  placeholder = "No file chosen"),
        tags$small("Maximum 3 files.", style = "color: #666;"),
        tags$br(),
        tags$small("2 MB limit. Allowed types: png, gif, jpg, jpeg.", style = "color: #666;")
      )
    ),
    fluidRow(
